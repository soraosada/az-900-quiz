...
- **問題数: 125問** — 公式 Skills measured（**2026-07-20版**）に対応させて配分
- 暗記カード: 31サービス（8カテゴリ）
- ※ 公式は実際の過去問を公開していないため、本アプリは公式出題範囲に沿った**オリジナル問題**。追加は `QUESTIONS` 配列に同形式で足すだけ。

## 出題形式の割合（本試験の形式ミックスに近似）
...
| 複数選択（2つ選べ） | 18問 | 14% |
| 正誤判定（はい/いいえ） | 14問 | 11% |
| 文章完成（___補充・ドロップダウン代替） | 10問 | 8% |

> 注: Microsoftは各形式の正確な出題割合を非公開。上記は一般的な体感に基づく目安。
...
|---|---|---|---|---|
| クラウドの概念 | 25–30% | 28 | 34問 | 27% |
| Azureのアーキテクチャとサービス | 35–40% | 37 | 47問 | 38% |
| Azureの管理とガバナンス | 30–35% | 35 | 44問 | 35% |
